...
var init_common_b3e8012f_esm = __esm({
  "node_modules/@firebase/firestore/dist/common-b3e8012f.esm.js"() {
    ...
    __PRIVATE_NumericTransformOperation = class extends TransformOperation {
      constructor(e, t) {
        super(), this.serializer = e, this.Ae = t;
      }
    };
    __PRIVATE_NumericIncrementTransformOperation = class extends __PRIVATE_NumericTransformOperation {
    };
    __PRIVATE_NumericMinimumTransformOperation = class extends __PRIVATE_NumericTransformOperation {
    };
...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
    ...
    en = "ComponentProvider";
    tn = /* @__PURE__ */ new Map();
    nn = "firestore.googleapis.com";
    rn = true;
...
var Ut, Ht2, DocumentSnapshot$1, QueryDocumentSnapshot$1, SnapshotMetadata, DocumentSnapshot, QueryDocumentSnapshot, QuerySnapshot;
var init_index_esm7 = __esm({
  "node_modules/@firebase/firestore/dist/index.esm.js"() {
    init_index_esm4();
    init_index_esm2();
    init_common_b3e8012f_esm();
    init_common_b3e8012f_esm();
    init_index_esm();
    init_bloom_blob_es2018();
    init_index_esm3();
...
var require_background = __commonJS({
  "background.js"() {
    ...
    var AUDIO_PAGE = chrome.runtime.getURL("audio.html");
    ...
    chrome.runtime.onInstalled.addListener(async (details) => {
      ...
      try {
        ...
        console.error("[FF] install error:", e);
      }
    });
      ...
      try {
        ...
        if (settings?.notificationsEnabled !== false) {
          chrome.notifications.create(`fs_${now}`, {
            type: "basic",
            iconUrl: chrome.runtime.getURL("icons/icon48.png"),
            title: "\u{1F3AF} Focus Session Started",
            message: `${state.sessionName} \xB7 ${duration} min`,
            priority: 1
          });
        }
      } catch (_) {
    ...
    async function stopFocusSession(completed) {
      ...
      try {
        ...
        const startMs = state.startTime || (state.endTime ? state.endTime - (state.duration || 25) * 6e4 : Date.now());
        const elapsed = Math.max(1, Math.round((Date.now() - startMs) / 6e4));
        const entry = {
          id: Date.now(),
          sessionName: state.sessionName || "Deep Work",
          duration: state.duration || 25,
          elapsed,
          completed,
        ...
        try {
          ...
          if (settings?.notificationsEnabled !== false) {
            chrome.notifications.create(`fe_${Date.now()}`, {
              ...
              message: completed ? `Focused ${elapsed} min on "${entry.sessionName}". Great work!` : `Session ended after ${elapsed} min.`,
              priority: 2
            });
          }
        } catch (_) {
    ...
    chrome.webNavigation.onBeforeNavigate.addListener(async (details) => {
      ...
      try {
        ...
        if (sessionActive) {
          ...
          const bl = [.../* @__PURE__ */ new Set([...userBl, ...globalBl, ...orgBl])];
          hit = bl.some((d) => host === d || host.endsWith("." + d));
        }
        if (hit) {
          const rem = sessionActive ? Math.max(0, Math.round((stored.focusState.endTime - Date.now()) / 6e4)) : 0;
          const params = new URLSearchParams({
            site: host,
