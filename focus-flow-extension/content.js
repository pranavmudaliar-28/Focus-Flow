// Focus Flow – content.js  v2.0
// Two responsibilities:
//   1. Show the thin focus-bar at the top of allowed pages during a session
//   2. Show a full session-complete overlay when the timer ends
//      (triggered by background.js → SESSION_COMPLETE message)
...
(function () {
  ...
  /* ══════════════════════════════════════════════════════
     FOCUS BAR  (thin gradient stripe at top of page)
  ══════════════════════════════════════════════════════ */
  function injectBar() {
    ...
    bar.style.cssText =
      ...
      'animation:ffp 2.5s ease-in-out infinite;';
  ...
  /* ══════════════════════════════════════════════════════
     ...
     Shows a beautiful full-page overlay when the session ends.
     User can dismiss it or start a new one.
  ══════════════════════════════════════════════════════ */
  function showSessionCompleteOverlay(data) {
    ...
    overlay.innerHTML = `
      <div id="ff-overlay-card">
        <span class="ff-icon">${isComplete ? '🎉' : '⏹'}</span>
        ...
        <p class="ff-sub">
          ...
          ${isComplete ? 'Amazing work! 🔥' : 'Every minute counts!'}
        </p>
        ...
        <div class="ff-stats">
          ...
          <div class="ff-stat">
            <div class="ff-stat-num">🔥</div>
            <div class="ff-stat-lbl">Streak</div>
          </div>
        </div>
        <div class="ff-btns">
          <button class="ff-btn-primary" id="ff-new-session">🚀 Start Another Session</button>
    ...
    // Remove bar since session is done
    removeBar();
    ...
    document.getElementById('ff-new-session')?.addEventListener('click', () => {
      dismiss();
      // Can't open popup programmatically, but we can signal it
  ...
  /* ══════════════════════════════════════════════════════
     ...
     Listens for SESSION_COMPLETE from background.js
  ══════════════════════════════════════════════════════ */
